...
export const metadata: Metadata = {
  title: "民間交易區",
  description: "瀏覽玩家刊登的配布寶可夢，以新台幣出價或立即購買。",
};
...
export default async function CommunityMarketPage() {
  ...
  const [{ data: activeRows, error: activeError }, { data: recentRows, error: recentError }] =
    await Promise.all([
      ...
      supabase
        .from("community_auctions")
        .select(MARKET_SELECT)
        .in("status", ["pending_payment", "sold", "ended", "cancelled"])
  ...
  if (user) {
    ...
    const listedIds = new Set((ownOpenRows || []).map((row) => row.user_distribution_id));
